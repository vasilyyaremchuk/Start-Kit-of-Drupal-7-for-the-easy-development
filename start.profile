...
function start_form_install_configure_form_alter(&$form, $form_state) {
  ...
  $form['#submit'] = array('start_custom_submit', 'install_configure_form_submit');
}

/**
 * Custom submit handler for the localized install.
 */
function start_custom_submit($form, &$form_state) {
...
function start_install_tasks($install_state) {
  ...
  return array(
    'start_import_translation' => array(
 ...
 */
function start_install_tasks_alter(&$tasks, $install_state) {
 ...
 */
function start_import_translation(&$install_state) {
